...
local function error_response(status, code, message, config)
    ...
    local jsonData = '{"success":"' .. code .. '", "message":"' .. message .. '"}'
    ngx.status = status
...
--
-- Check for a required application if configured
--
local function verify_application(jwt_text, required_application)
    
    if required_application == nil then
        return true
    end
    ...
    local application_values = pl_stringx.split(data.claims.application, " ")
    for id, application in ipairs(application_values) do
        if application == required_application then
            return true
        end
    end
...
function _M.run(config)
    ...
    if not verify_application(jwt, config.application) then
        error_response(ngx.HTTP_FORBIDDEN, "forbidden", "The token does not contain the required scope: " .. config.application, config)
    end
